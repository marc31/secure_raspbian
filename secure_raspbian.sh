...
function duckdns() {
	...
	if [[ $BOTASK_ANSWER == 1 ]]; then
		...
		if sudo crontab -l | grep -Fq "01 00 * * * curl -k -o /tmp/duckdns.log \"https://www.duckdns.org/update?domains=$DUCKDNSDOMAIN&token=$DUCKDNSTOKEN&ip=\" >/dev/null 2>&1"
		then
			say_red "DuckDns cron job is already exists"
		else
			(sudo crontab -l && echo "01 00 * * * curl -k -o /tmp/duckdns.log \"https://www.duckdns.org/update?domains=$DUCKDNSDOMAIN&token=$DUCKDNSTOKEN&ip=\" >/dev/null 2>&1") | sudo crontab -
		fi
